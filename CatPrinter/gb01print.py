...
def detect_printer(detected, advertisement_data):
    global device
    if detected:
        print ("device address: %s" % detected.address)
    if address:
...
def drawText(text, from_file=False, font=None):
    text_chunks = []
    if font is None:
    ...
    if len(text) > max_line_length and not from_file:
        text_chunks = [text[i:i+max_line_length] for i in range(0, len(text), max_line_length)]
        text = '\n'.join(text_chunks)

    img = PIL.Image.new('RGBA', (10,PrinterWidth), (255, 255, 255, 0))
...
textfile_args = parser.add_mutually_exclusive_group()

textfile_args.add_argument("-f", "--textfile", 
                       type=str, default=text_to_print, metavar="TEXTFILE",
                       help="Print some text from a text file")

text_args = parser.add_mutually_exclusive_group()

text_args.add_argument("-t", "--text", 
                       type=str, default=text_to_print, metavar="TEXT",
                       help="Print some text")

image_args = parser.add_mutually_exclusive_group()

image_args.add_argument("-i", "--image", 
                       type=str, default=None, metavar="IMAGE",
                       help="Print an image")
...
feed_args.add_argument("-p", "--feed", type=int, default=feed_lines, metavar="LINES",
...
throttle_args.add_argument("-s", "--throttle", type=float, default=throttle, metavar="SECONDS",
                           help="delay between sending command queue packets (default: {})".format(throttle),)

throttle_args.add_argument("-S", "--no-throttle",
                           help="don't wait while sending data",
                           action="store_const", dest="throttle", const=None)

parser.add_argument("--packetsize", type=int, default=packet_length, metavar="BYTES",
...
if not args.eject:
    ...
    if args.image:
        ...
        text_file = open(args.textfile)
        text_to_print = text_file.read()
        image = drawText(text_to_print, from_file=True)
        print_data = print_data + render_image(image)
    elif args.text:
        text_to_print = args.text
        image = drawText(text_to_print)
        print_data = print_data + render_image(image)
