...
class ImageTreeprocessor(markdown.treeprocessors.Treeprocessor):
    ...
    def get_image(self):
        ...
        final = Image.new("RGBA", (self.image_width, height), self.config["background_color"])
        y = 0
    ...
    def new_image_block(self):
        ...
        self.image = Image.new("RGBA", (self.image_width, IMAGE_BLOCK_HEIGHT), self.config["background_color"])
    ...
    def render_text(self, text, color, end_block=False, font=None):
        ...
        if self.in_pre:
            ...
            while end_index < len(parts):
                w = 0
                while end_index < len(parts):
                    (w, h) = draw.textsize(text=" ".join(parts[start_index:end_index + 1]),
                                           font=font)
